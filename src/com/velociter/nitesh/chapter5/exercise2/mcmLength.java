...
/*
Define a class, mcmLength, to represent a length measured in meters, centimeters, and millimeters, each stored as integers.Include methods to add and subtract 
objects, to multiply and divide an object by an integer value, to calculate an area resulting from the product of two objects, and to compare objects. Include 
constructors that accept three arguments — meters, centimeters, and millimeters; one integer argument in millimeters; one double argument in centimeters; and 
no arguments, which creates an object with the length set to zero. Check the class by creating some objects and testing the class operations.*/
